...
Внешние ID разных платформ не нужны и отдельных таблиц под них не требуется. Нормализация разрешает относительные URL, удаляет фрагмент, приводит схему/хост и удаляет согласованные tracking-параметры. Если устойчивой ссылки нет, YAML задаёт fallback-фингерпринт; изменение его состава считается новой вакансией.

## История и запуски

`vacancy_history` содержит `id`, `vacancy_id`, `field_name`, `old_value`, `new_value`, `changed_at`. В первом релизе отслеживаются `title`, `company`, `salary`, `link`; `description` только сохраняется.
